...
const removePromo = async (req, res) => {
    const { error, value } = (0, validateRequest_1.validateRequest)(promoSchema_1.deletePromoSchema, req.query);
    if (error != null) {
    ...
    const { promoId } = value;
    try {
        const promotion = await promotions_1.PromoModel.findOne({
            where: {
        ...
        logger_1.default.info(`Deleted promotion with ID: ${promoId} successfully`);
