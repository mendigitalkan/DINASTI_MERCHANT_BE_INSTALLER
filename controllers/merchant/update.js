...
const updateMerchant = async (req, res) => {
    const { error, value } = (0, validateRequest_1.validateRequest)(merchantSchema_1.updateMerchantSchema, req.body);
    if (error != null) {
    ...
    try {
        const merchant = await merchantModel_1.MerchantModel.findOne({
            where: {
                ...
                merchantId: { [sequelize_1.Op.eq]: value.merchantId }
            }
        });
        if (merchant == null) {
        ...
        logger_1.default.info(`Updated merchant with ID: ${value.certificateId} successfully`);
