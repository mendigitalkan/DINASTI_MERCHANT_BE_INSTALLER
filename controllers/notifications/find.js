...
const findAllNotification = async (req, res) => {
    const { error, value } = (0, validateRequest_1.validateRequest)(notificationSchema_1.findAllNotificationSchema, req.query);
    if (error != null) {
    ...
    try {
        ...
        const notifications = await notifications_1.NotificationModel.findAndCountAll({
            where: {
                ...
                ...(Boolean(search) && {
                    [sequelize_1.Op.or]: [{ notificationName: { [sequelize_1.Op.like]: `%${search}%` } }]
                })
            },
            order: [['id', 'desc']],
            ...(pagination === 'true' && {
        ...
        logger_1.default.info('Fetched all notifications successfully');
...
const findDetailNotification = async (req, res) => {
    ...
    try {
        ...
        logger_1.default.info('Fetched notification details successfully');
        return res.status(http_status_codes_1.StatusCodes.OK).json(response);
    }
    catch (error) {
        const message = `Unable to process request! Error: ${error.message}`;
